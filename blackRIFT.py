...
def looks_like_dns_domain(host):
    name = normalize_dns_name(host)
    if not name or "/" in name or ":" in name or name == "localhost" or host_is_ip(name):
        return False
    return "." in name and all(part for part in name.split("."))


def should_auto_subfinder(raw_target):
    host, embedded_port = parse_host_port(raw_target)
    return embedded_port is None and looks_like_dns_domain(host)
...
def is_host_in_domain(host, domain):
...
def run_subfinder(domain, timeout):
    if not shutil.which("subfinder"):
        raise RuntimeError("subfinder was not found in PATH")
    ...
    try:
        ...
        raise RuntimeError(f"subfinder timed out after {timeout:g}s") from exc
    if completed.returncode != 0 and not completed.stdout.strip():
        detail = completed.stderr.strip() or f"exit status {completed.returncode}"
        raise RuntimeError(f"subfinder failed: {detail}")
    hosts = []
    seen = set()
    for line in completed.stdout.splitlines():
        host = normalize_dns_name(line)
        if host.startswith("*."):
            host = host[2:]
        if not looks_like_dns_domain(host) or not is_host_in_domain(host, domain):
            continue
        if host in seen:
            continue
        seen.add(host)
...
def assess(args):
    ...
    assessment = Assessment(
        target={
            ...
            "discovery": getattr(args, "discovery", {}),
        },
        started_at=now(),
    )
    ...
    artifact_name = f"nginx_rifter_{safe_filename(args.host)}_{args.port}_{timestamp}.json"
    artifact_path = args.output or os.path.join(
        args.artifact_dir,
        f"nginx_rifter_{dt.datetime.now().strftime('%Y%m%d-%H%M%S')}.json",
    )
    if getattr(args, "multi_target_mode", False) and not args.output:
        artifact_path = os.path.join(args.artifact_dir, artifact_name)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        usage="%(prog)s --target HOST[:PORT]|DOMAIN [--file-read-template TEMPLATE] [options]",
        description=(
    ...
    target.add_argument("--target", default=DEFAULT_TARGET, help="target as HOST:PORT, host, or domain for subfinder fan-out")
    ...
    target.add_argument("--subfinder", dest="subfinder", action="store_true", default=None, help="force subfinder -d <domain> -all before assessments")
    target.add_argument("--no-subfinder", dest="subfinder", action="store_false", help="disable automatic subfinder for domain-only targets")
    target.add_argument("--subfinder-timeout", type=float, default=120.0, help="seconds to wait for subfinder")
    target.add_argument("--subfinder-max-hosts", type=int, default=0, help="maximum discovered hosts to assess; 0 means unlimited")
    target.add_argument("--subfinder-output", help="optional path to write discovered hosts")
    ...
    exploit.add_argument("--allow-multi-exploit", action="store_true", help=advanced("allow --exploit after subfinder fan-out"))
    ...
    parser.set_defaults(auto_calibrate=True, cleanup_core=True, include_apex=True)
    ...
    args.host = host
    args.port = port
    args.subfinder_domain = normalize_dns_name(host)
    args.use_subfinder = args.subfinder is True or (
        args.subfinder is None and should_auto_subfinder(args.target)
    )
    if args.use_subfinder and not looks_like_dns_domain(args.subfinder_domain):
        parser.error("--subfinder requires --target to be a DNS domain")
    if args.subfinder_timeout <= 0:
        parser.error("--subfinder-timeout must be positive")
    if args.subfinder_max_hosts < 0:
        parser.error("--subfinder-max-hosts must be zero or positive")
    if args.exploit and args.use_subfinder and not args.allow_multi_exploit:
        parser.error("--exploit with subfinder fan-out requires --allow-multi-exploit")
...
def subfinder_targets(args):
    domain = args.subfinder_domain
    discovered = run_subfinder(domain, args.subfinder_timeout)
    hosts = []
    seen = set()
    ...
    for host in discovered:
        ...
        seen.add(host)
        hosts.append(host)
    if args.subfinder_max_hosts:
        hosts = hosts[: args.subfinder_max_hosts]
    if not hosts:
        raise RuntimeError("subfinder did not return any usable hosts")
    if args.subfinder_output:
        write_lines(args.subfinder_output, hosts)
    return hosts, len(discovered)


def run_with_optional_subfinder(args):
    if not args.use_subfinder:
        return assess(args)

    c = Palette(not args.no_color)
    print(c.bold(c.cyan("[subfinder] discovering targets")), flush=True)
    print(f"    domain:  {args.subfinder_domain}", flush=True)
    print("    command: subfinder -d <domain> -all -silent", flush=True)
    hosts, discovered_count = subfinder_targets(args)
    print(f"    found:   {discovered_count} subfinder host(s)", flush=True)
    print(f"    queued:  {len(hosts)} assessment target(s) on port {args.port}", flush=True)
    if args.subfinder_output:
        print(f"    output:  {args.subfinder_output}", flush=True)

    if args.output and len(hosts) > 1:
        raise RuntimeError("--output can only be used with one target; use --artifact-dir for subfinder fan-out")
    ...
    base_discovery = {
        ...
        "queued_count": len(hosts),
        "output": args.subfinder_output or "",
    }
    exit_code = 0
    for index, host in enumerate(hosts, start=1):
        current = copy.copy(args)
        current.host = host
        current.target = f"{host}:{args.port}"
        current.discovery = dict(base_discovery, index=index)
        current.multi_target_mode = len(hosts) > 1
        if current.multi_target_mode:
            current.output = None
        print(c.bold(c.magenta(f"\n[subfinder] target {index}/{len(hosts)}: {host}:{args.port}")), flush=True)
        try:
            target_exit = assess(current)
        except Exception as exc:
            failure_label = c.red("nginx_rifter failed for")
            print(f"\n{failure_label} {host}:{args.port}: {exc}", file=sys.stderr, flush=True)
            target_exit = 1
        if target_exit and exit_code == 0:
